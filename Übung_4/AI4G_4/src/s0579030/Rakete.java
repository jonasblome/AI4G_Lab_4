...
public class Rakete extends AI {
	...
	public Rakete(Info info) {
		...
		sortPearls();
		sortRecyclingProducts();
		setGrid();
		setNeighbours();
		setSurfacePearlPaths();
		setNextPearlPaths();
		setSurfaceProductPaths();
	}
	...
	// Sort recycling products by distance to start position
	public void sortRecyclingProducts() {
		// Sort pearls by distance to diver position value with bubble sort
		for(int i = 0; i < products.length - 1; i++) {
			for(int j = 0; j < products.length - i - 1; j++) {
				Vector2D diverPosition = getDiverPosition();

				int currentBottleDiverDistance = (int) getProductPosition(j).subtractVector(diverPosition).getLength();
				int nextBottleDiverDistance = (int) getProductPosition(j + 1).subtractVector(diverPosition).getLength();

				if (currentBottleDiverDistance > nextBottleDiverDistance) {
					Point2D temp = products[j];
	...
	public void setSurfacePearlPaths() {
		...
		for(int i = 0; i < pearls.length; i++) {
			...
			pathsSurfacePearlAirUsages[i] = calculateAirUsage(pathFromSurface, errorMargin);
	...
	public void setSurfaceProductPaths() {
		System.out.println("Setting paths to surface...");
		for(int i = 0; i < products.length; i++) {
			...
			pathsSurfaceProductAirUsages[i] = calculateAirUsage(pathFromSurface, errorMargin);
	...
	public ArrayList<Vector2D> setPathAroundShip(int radius) {
		...
		sortRecyclingProducts();
		Vector2D diverPosition = shipPosition;
		ArrayList<Point2D> uncollectedProducts = new ArrayList<>();
		int bottleSearchRadius = radius;
		int productsInRadius = 0;

		while(productsInRadius < 6 - (itemsBought * 2 + info.getMoney()) && bottleSearchRadius < 300) {
			productsInRadius = 0;
		...
		for(int rp = 0; rp < productsInRadius - 1; rp++) {
			Vector2D productPosition = getProductPosition(rp);
			ArrayList<Vector2D> pathToCurrentProduct = aStarPathFinding(diverPosition, productPosition);
			pathToCurrentProduct.add(productPosition);
			pathToCurrentProduct = smoothPath(pathToCurrentProduct);

			int productShipDistance = (int) productPosition.subtractVector(shipPosition).getLength();

			if(productShipDistance < bottleSearchRadius) {
				pathAroundShip.addAll(pathToCurrentProduct);
				diverPosition = productPosition;
			}
			else {
				uncollectedProducts.add(products[rp]);
		...
		ArrayList<Vector2D> bottleShipPath = aStarPathFinding(diverPosition, shipPosition);
		bottleShipPath = smoothPath(bottleShipPath);
		pathAroundShip.addAll(bottleShipPath);
		...
		for(int i = 0; i < pearls.length - 1;) {
			...
			if(i < pearls.length - 1) {
				...
				for (int j = i; j < pearls.length - 2 && !partialPathDone; j++) {
					if (info.getMaxAir() > calculateAirUsage(partialPath, errorMargin) + pathsSurfacePearlAirUsages[j + 1]) {
